...
public class ThaumCraft extends ProtBase
{
	...
	private Class clAlumentum = null, clTileArcaneBore, 
			clEntityFrostShard, clItemWandFire, clItemWandExcavation,
			clItemWandLightning, clItemWandTrade;
	...
	public void load() throws Exception
	{
		...
		clTileArcaneBore = Class.forName("thaumcraft.common.blocks.TileArcaneBore");
	...
	@Override
	public boolean isEntityInstance(Entity e) { return clAlumentum.isInstance(e) || clEntityFrostShard.isInstance(e); }
	@Override
	public boolean isEntityInstance(TileEntity e) { return e.getClass() == clTileArcaneBore; }
	@Override
	...
	public String update(Entity e) throws Exception
	{
		if (clAlumentum.isInstance(e))
		{
			...
			if (!thrower.canInteract(dim, x - explosionRadius, y, z - explosionRadius, Permissions.Build) || 
		...
		else if (clEntityFrostShard.isInstance(e))
		{
			...
			if (!thrower.canInteract(dim, x - radius, y - radius, y + radius, z - radius, Permissions.Build) || 
					!thrower.canInteract(dim, x - radius, y - radius, y + radius, z + radius, Permissions.Build) ||
					!thrower.canInteract(dim, x + radius, y - radius, y + radius, z - radius, Permissions.Build) ||
					!thrower.canInteract(dim, x + radius, y - radius, y + radius, z + radius, Permissions.Build))
	...
	public String update(Resident res, Item tool, ItemStack item) throws Exception
	{
		...
		else if (clItemWandTrade.isInstance(tool))
		{
			if (!res.onlinePlayer.isSneaking())
			{
				...
				if (pos != null && pos.typeOfHit == EnumMovingObjectType.TILE)
				{
					int x = (int) (pos.blockX);
					int y = (int) (pos.blockY);
					int z = (int) (pos.blockZ);
					int radius = 3;
					int dim = res.onlinePlayer.dimension;

					if (!res.canInteract(dim, x - radius, y - radius, y + radius, z - radius, Permissions.Build) || 
	...
	public String update(TileEntity e) throws Exception
	{
		if (clTileArcaneBore.isInstance(e))
		{
			...
			if (fBore_toDig.getBoolean(e))
			{
				...
				{
					Log.warning(String.format("Thaumcraft bore at Dim %s (%s,%s,%s) tried to break (%s,%s,%s) which failed. Actor: %s", 
							e.worldObj.provider.dimensionId, e.xCoord, e.yCoord, e.zCoord, 
							fBore_digX.getInt(e), fBore_digY.getInt(e), fBore_digZ.getInt(e),
							actor.name()));
